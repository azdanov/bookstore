...
public class MockOAuth2UserContextFactory implements WithSecurityContextFactory<WithMockOAuth2User> {
    ...
    public SecurityContext createSecurityContext(WithMockOAuth2User withUser) {
        ...
        List<GrantedAuthority> authorities = Stream.of(withUser.roles())
                .map(MockOAuth2UserContextFactory::toGrantedAuthority)
                .collect(Collectors.toList());

        Map<String, Object> claims = Map.of(
                "preferred_username", username,
                "userId", withUser.id(),
                "realm_access", authorities);
    ...
    private static SimpleGrantedAuthority toGrantedAuthority(String role) {
